...
from tapas_gmm_modified.policy.models.diffusion.positional_embedding import (
    SinusoidalPosEmb,
)
...
class ConditionalUnet1D(nn.Module):
    def __init__(
        ...
        cond_dim = dsed
        if config.global_cond_dim is not None:
            cond_dim += config.global_cond_dim

        in_out = list(zip(all_dims[:-1], all_dims[1:]))

        local_cond_encoder = None
        if config.local_cond_dim is not None:
            _, dim_out = in_out[0]
            dim_in = config.local_cond_dim
